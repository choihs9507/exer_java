...
public class Lotto6Num {
	...
	public Lotto6Num() { //공의 개수 45, 배열 크기 6으로 초기화
		this(45,6); //?왜 변수 바로 안넣고 this로??
	}
	...
	//임의의 정수 만들기
	private int rand() {
		return (int)(Math.random()*base)+1; //randon()은 0.xxxx ~ 0.99999
	}
	//n이 배열에 포함되는지 판단, 중복된 수 배제하기 위해
	...
	public void make() {
		...
		while(count!=ballNum) { //6개의 다른 정수가 채워질 때까지 반복
			int temp=rand(); //1~45사이 임의 정수 만들기
			if(!contain(temp)) { //배열에 포함되어 있지 않으면 배열에 저장, 저장 개수 증가
				lots[count++]=temp;
			}
		}
